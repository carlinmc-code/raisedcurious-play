...
/* The iOS launch images are decoration for the moment the app opens, and
   they weigh more than the entire rest of the site. Precaching them would
   double the plane-mode download to remove a white flash. They are left to
   the runtime cache instead: a device fetches only its own one, once. */
const NOT_OFFLINE = /\/assets\/icons\/splash-/;
...
function referenced(files){
  ...
  for (const f of files){
    ...
    while ((m = re.exec(text))){
      ...
      if (NOT_OFFLINE.test(url)) continue;
      found.add(url);
    }
  }
  return found;
...
for (const f of files){
  if (NOT_OFFLINE.test(f)) continue;
  urls.add(f);
...
/* Size of what actually gets downloaded, which is not the size of the
   folder: the launch images are on the server but never precached. */
const bytes = files.filter(f => !NOT_OFFLINE.test(f))
  .reduce((n, f) => n + fs.statSync(path.join(ROOT, f.slice(1))).size, 0);
const out =
