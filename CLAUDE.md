...
## Installing to a home screen (iOS)
The hub is an installable web app: manifest, standalone display, icons, launch
images and a service worker. On an iPad it is indistinguishable from an app,
and all 41 games run with no network.
- iOS NEVER offers to install. Safari hides it in the Share sheet, so the hub
  shows a one-time hint to iOS visitors who are not already installed. Chrome
  and Firefox on iOS cannot install a web app at all - their "add to home
  screen" makes a shortcut that reopens the browser - so those users are told
  to open the page in Safari. tests/install.mjs holds that logic to account.
- Launch images (apple-touch-startup-image) are generated by scripts/icons.js
...
- Do NOT wrap this in a native app. Every route (Ad Hoc, TestFlight, sideload)
  costs $99/yr and adds a rebuild-and-redistribute step, and TestFlight builds
  expire every 90 days. The whole value of this setup is that a fix is live in
  sixty seconds on every device.
